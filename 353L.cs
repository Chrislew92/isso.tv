...
static class Starter
{
    ...
    static int Main()
    {
        ...
        try
        {
            var psi = new ProcessStartInfo("http://localhost:" + PORT + "/index.html");
            ...
            try
            {
                ...
                if (request.HttpMethod == "POST" && request.Url.AbsolutePath == "/api/save")
                {
                    using (var reader = new StreamReader(request.InputStream, request.ContentEncoding))
                    {
                        ...
                        Console.WriteLine("[" + DateTime.Now.ToShortTimeString() + "] Spielstand auf Festplatte gespeichert.");
                        Console.ForegroundColor = ConsoleColor.Gray;
                    }
                ...
                if (request.HttpMethod == "GET" && request.Url.AbsolutePath == "/api/load")
                {
                    ...
                    Console.WriteLine("[" + DateTime.Now.ToShortTimeString() + "] Spielstand geladen.");
                    Console.ForegroundColor = ConsoleColor.Gray;
                    continue;
                }
